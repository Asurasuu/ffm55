...
if( $_SESSION['id'] == "" ) {
    ...
    if( $user['status'] != 10 ) {
        ...
        $persons_query = mysqli_query($connection, "SELECT * FROM `users`");
        $count = 0; // Счётчик пользователей
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Омская Федерация Функционального Многоборья - Все пользователи</title>
...
<body>
    ...
    <div class="content_wrapper">
        <h1 class="content_title">Пользователи</h1>
        <div class="user block">
            <div class="content_button">
                <a href="./adm_users_add.php">Добавить пользователя</a>
            </div>
            <div class="content_table">
                <div class="row">
                    <div class="date main-cell">Номер</div>
                    <div class="title main-cell">ФИО</div>
                    <div class="edit main-cell">Изменить</div>
                    <div class="delete main-cell">Удалить</div>
                </div>

                <?
                    while( $person = mysqli_fetch_array($persons_query) ) {
                        $count++;

                        ?>
                        <div class="row">
                            <div class="date cell cell-center"><? echo $count; ?></div>
                            <div class="title cell"><a href="./adm_user_info.php?id=<? echo $person['id'];?>"><? echo $person['surname'] . ' ' . $person['name'] . ' ' . $person['lastname']; ?></a></div>
                            <div class="edit cell cell-center"><!-- <a href="#">Изменить</a> --></div>
                            <div class="delete cell cell-center"><a href="./scripts/delete_user.php?id=<? echo $person['id']?>">Удалить</a></div>
                        </div>
                        <?
                    } 
                ?>
